    ...
    size_t nbytes, off_t offset, size_t *nread)
{
	...
	while (done < nbytes) {
		...
		bzero(&data, sizeof (data));
		data.mlrd_mcia.mlrd_mcia_module = id;
...
mlxcx_mac_ring_tx(void *arg, mblk_t *mp)
{
	...
	off = 0;
	while (rem > 0) {
		const size_t sz = kmp->b_wptr - kmp->b_rptr;
		take = sz;
		if (take > rem)
			take = rem;
    ...
    uint_t pr_valsize, const void *pr_val)
{
	...
	case MAC_PROP_MTU:
		...
		/* XXX: setting hw mtu to mtu+22: is there a better way? */
		new_hw_mtu = new_mtu + 22;
    ...
    uint_t pr_valsize, void *pr_val)
{
	...
	case MAC_PROP_DUPLEX:
		...
		/* XXX: pretty sure connectx parts only support full duplex */
	...
	case MAC_PROP_MTU:
		...
		/* XXX: we currently set the hw mtu to 22 more than MAC */
		*(uint32_t *)pr_val = port->mlp_mtu - 22;
		break;
	default:
		ret = ENOTSUP;
		break;
