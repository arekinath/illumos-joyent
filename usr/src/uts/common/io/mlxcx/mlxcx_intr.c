...
mlxcx_report_module_error(mlxcx_t *mlxp, mlxcx_evdata_port_mod_t *evd)
{
	...
	if (evd->mled_port_mod_module_status != MLXCX_MODULE_ERROR)
		return;

	name = mlxcx_module_error_string(evd->mled_port_mod_error_type);

	(void) snprintf(buf, FM_MAX_CLASS, "%s.%s.%s", DDI_IO_CLASS,
	    DDI_FM_SERVICE_DEGRADED, "txr");
	ena = fm_ena_generate(0, FM_ENA_FMT1);
	if (!DDI_FM_EREPORT_CAP(mlxp->mlx_fm_caps))
		return;

	mlxcx_warn(mlxp, "transceiver module %u error: %s (0x%x)",
	    evd->mled_port_mod_module, name, evd->mled_port_mod_error_type);
	ddi_fm_ereport_post(mlxp->mlx_dip, buf, ena, DDI_NOSLEEP,
	    FM_VERSION, DATA_TYPE_UINT8, FM_EREPORT_VERS0,
	    "error", DATA_TYPE_STRING, name,
	    "error_num", DATA_TYPE_UINT8, evd->mled_port_mod_error_type,
	    "transceiver_index", DATA_TYPE_UINT8, evd->mled_port_mod_module,
	    NULL);
}
