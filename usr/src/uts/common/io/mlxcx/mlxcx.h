...
/*
 * Get access to the first PCI BAR. XXX Probably
 */
...
#define	MLXCX_RX_NGROUPS_SMALL_DFLT		128
...
#define	MLXCX_TX_NRINGS_PER_GROUP_DFLT	32
...
#define	MLXCX_CQEMOD_COUNT_DFLT 	(8 * \
	((1 << MLXCX_CQ_SIZE_SHIFT_DFLT) / 10))
 ...
 * other problems.
 */
#define	MLXCX_CQ_CHECK_INTERVAL_NSEC		15000000000
...
typedef enum {
	...
	MLXCX_DMABUF_COOKIES		= 1 << 3,
	MLXCX_DMABUF_FOREIGN		= 1 << 4,
...
typedef struct mlxcx_dma_buffer {
	...
	size_t				mxdb_cookies_size;
	ddi_dma_cookie_t		*mxdb_cookies;
} mlxcx_dma_buffer_t;
...
typedef struct mlxcx_event_queue {
	...
	boolean_t		mleq_hascheck;
	ddi_periodic_t		mleq_checktimer;
} mlxcx_event_queue_t;
...
typedef struct mlxcx_completion_queue {
	...
	ddi_periodic_t			mlcq_checktimer;
...
struct mlxcx {
	 ...
	 */
	uint_t			mlx_intr_pri;
	uint_t			mlx_intr_type;		/* XXX: always MSI-X */
	int			mlx_intr_count;
    ...
    const ddi_dma_cookie_t *prev)
{
	...
	ASSERT(db->mxdb_flags & MLXCX_DMABUF_COOKIES);
	if (prev == NULL) {
		return (&db->mxdb_cookies[0]);
	}
	idx = prev - db->mxdb_cookies;
	ASSERT3U(idx, <=, db->mxdb_ncookies);
	if (++idx >= db->mxdb_ncookies) {
		return (NULL);
	}
	return (&db->mxdb_cookies[idx]);
}
...
extern boolean_t mlxcx_cmd_set_flow_table_entry(mlxcx_t *, mlxcx_flow_entry_t *);
