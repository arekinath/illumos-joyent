...
/*
 * DMA allocation and tear down routines. The purpose of these is to 
 */
...
{
	if (mxdb->mxdb_flags & MLXCX_DMABUF_COOKIES) {
		VERIFY(mxdb->mxdb_cookies != NULL);
		kmem_free(mxdb->mxdb_cookies, mxdb->mxdb_cookies_size);
		mxdb->mxdb_cookies = NULL;
		mxdb->mxdb_cookies_size = 0;
		mxdb->mxdb_ncookies = 0;
		mxdb->mxdb_flags &= ~MLXCX_DMABUF_COOKIES;
	}
	...
	ASSERT3U(mxdb->mxdb_cookies_size, ==, 0);
	ASSERT3P(mxdb->mxdb_cookies, ==, NULL);
}
...
mlxcx_dma_unbind(mlxcx_t *mlxp, mlxcx_dma_buffer_t *mxdb)
{
	...
	if (mxdb->mxdb_flags & MLXCX_DMABUF_COOKIES) {
		VERIFY(mxdb->mxdb_cookies != NULL);
		mxdb->mxdb_ncookies = 0;
		bzero(mxdb->mxdb_cookies, mxdb->mxdb_cookies_size);
	}
    ...
    const mblk_t *mp, size_t off, boolean_t wait)
{
	...
	needcksize = ncookies * sizeof (ddi_dma_cookie_t);
	if (mxdb->mxdb_cookies_size < needcksize) {
		kmem_free(mxdb->mxdb_cookies, mxdb->mxdb_cookies_size);
		mxdb->mxdb_cookies_size = ncookies * sizeof (ddi_dma_cookie_t);
		mxdb->mxdb_cookies = kmem_zalloc(mxdb->mxdb_cookies_size,
		    wait ? KM_SLEEP : KM_NOSLEEP);
		if (mxdb->mxdb_cookies == NULL) {
			mxdb->mxdb_cookies_size = 0;
			mlxcx_dma_unbind(mlxp, mxdb);
			return (B_FALSE);
		}
	}
	mxdb->mxdb_cookies[0] = cookie;
	for (i = 1; i < ncookies; ++i) {
		ddi_dma_nextcookie(mxdb->mxdb_dma_handle,
		    &mxdb->mxdb_cookies[i]);
	}
	mxdb->mxdb_ncookies = ncookies;
	mxdb->mxdb_flags |= MLXCX_DMABUF_COOKIES;
    ...
    size_t size, boolean_t wait)
{
	...
	ret = ddi_dma_addr_bind_handle(mxdb->mxdb_dma_handle, NULL,
	    mxdb->mxdb_va, len, DDI_DMA_RDWR | flags, memcb, NULL, &cookie,
	    &ncookies);
	if (ret != 0) {
