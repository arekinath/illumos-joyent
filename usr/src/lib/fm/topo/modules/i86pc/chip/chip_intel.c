    ...
    nvlist_t *dimm_nvl, uint64_t rsize, uint32_t id)
{
	...
	if (topo_node_asru_set(rank, fmri, TOPO_ASRU_COMPUTE, &err) != 0) {
		whinge(mod, NULL, "mc_rank_crate_v1: failed to set asru: %d",
		    err);
	...
	/*
	 * The traditional northbridge driver broken down each rank into the
	 ...
	 */
	if (topo_prop_set_uint64(rank, PGNAME(RANK), RANK_SIZE,
	    TOPO_PROP_IMMUTABLE, rsize, &err) != 0) {
		whinge(mod, NULL, "mc_create_rank_v1: failed to set %s "
	...
	if (nvlist_lookup_boolean_array(dimm_nvl, MCINTEL_NVLIST_V1_DIMM_RDIS,
		...
		whinge(mod, NULL, "mc_create_rank_v1: Couldn't find disabled "
		    "ranks array");
		return (topo_mod_seterrno(mod, EMOD_UNKNOWN));
	}

	if (id >= ndisabled) {
		whinge(mod, NULL, "mc_create_rank_v1: Found rank %u with id "
    ...
    nvlist_t *dimm_nvl, uint_t id)
{
	...
	rsize = size / banks;
	for (i = 0; i < ranks; i++) {
		if (mc_rank_create_v1(mod, dimm, auth, dimm_nvl, rsize, i) != 0) {
			return (-1);
		}
	}
